...
def load_model() -> keras.Model:
    model = get_model()
    model_weights = os.path.join(output_dir, "weights/keras.hdf5")
    # keras = keras.models.load_model(model_weights)
...
def load_my_test(staining: str, lim: int):
    test_folder = os.path.join(params.DATASET_PATH, "../../ims")
    ims = [i for i in glob.glob(test_folder + '/*') if staining in i]
    import random
    return random.sample(ims, lim)


def load_my_gt(ims: List[str]):
    gt_folder = os.path.join(params.DATASET_PATH, 'gt/masks')
    names = [os.path.basename(i) for i in ims]
    return [os.path.join(gt_folder, i) for i in names]
...
def get_predictions(staining: str):
    import shutil
    dataset_dir = "dataset_ines"
    os.mkdir(dataset_dir)
    staining_dir = os.path.join(dataset_dir, staining)
    os.mkdir(staining_dir)
    ims_dir = os.path.join(staining_dir, "tissue")
    os.mkdir(ims_dir)
    gt_dir = os.path.join(staining_dir, "groundtruth")
    os.mkdir(gt_dir)
    pred_dir = os.path.join(staining_dir, "predictions")
    os.mkdir(pred_dir)
    ...
    for im_file, gt_file in zip(test_files, gt_files):
        ...
        gt_file_dest = os.path.join(gt_dir, os.path.basename(gt_file))
        shutil.copyfile(im_file, im_file_dest)
        shutil.copyfile(gt_file, gt_file_dest)
        pred = _get_pred_mask(im, org_size, model, th)
        pred_pil = Image.fromarray(pred * 255)
        pred_pil.save(os.path.join(pred_dir, os.path.basename(im_file)))


def validate_model():
    """ Load keras weights and make predictions, optionally varying the output threshold. """
    model = load_model()
